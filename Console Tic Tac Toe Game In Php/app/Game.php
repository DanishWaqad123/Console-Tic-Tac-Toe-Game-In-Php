	...
	public function checkForWinOrTie($board)
	{
		...
		if($movesTillNow<=25){
			...
			for ($i=0; $i < 5; $i++) { 
				...
				&&($boardCells[$i][2]==$boardCells[$i][3])&&($boardCells[$i][3]==$boardCells[$i][4])){
				...
				&&($boardCells[2][$i]==$boardCells[3][$i])&&($boardCells[3][$i]==$boardCells[4][$i])){
			...
			&&($boardCells[2][2]==$boardCells[3][3]&&($boardCells[3][3]==$boardCells[4][4]))){
		...
		if(($movesTillNow==25)&&($this->getWinner()==null)){
			//set game over
			$this->setGameOver(true);
		}
	}
